...
export default function OutletDetail() {
  ...
  return (
    <main style={{ background: 'var(--clr-offwhite)', minHeight: '100vh', paddingBottom: '3rem' }}>
      <SEO 
      ...
      {/* Visual Header */}
      <div style={{ background: 'var(--bg-dark-surface)', borderBottom: '1px solid var(--border-on-dark)' }}>
        <div className="container" style={{ padding: '0' }}>
           <HighwayComposition style={{ width: '100%', height: 'auto', maxHeight: '400px', objectFit: 'cover' }} />
        </div>
      </div>
      ...
      <div className="container" style={{ marginTop: '-2rem', position: 'relative', zIndex: 10 }}>
        ...
        {/* Breadcrumb */}
        <div className="breadcrumb" style={{ background: 'var(--clr-white)', padding: '0.5rem 1rem', borderRadius: 'var(--radius-full)', display: 'inline-flex', boxShadow: 'var(--shadow-sm)', marginBottom: '1.5rem' }}>
          <Link to="/outlets">Outlets</Link>
          <span className="breadcrumb-sep">/</span>
          <span style={{ color: 'var(--txt-dark)', fontWeight: '600' }}>{outlet.id.toUpperCase()}</span>
        </div>
        ...
        <div className="grid-2" style={{ gap: '2.5rem', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ background: 'var(--clr-white)', padding: '2.5rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-light)' }}>
              <span className="status-badge status-badge--op" style={{ marginBottom: '1rem' }}>
                <span className="status-dot"></span>
                Operational
              </span>
              <h1 style={{ marginBottom: '0.5rem' }}>{outlet.title}</h1>
              <p className="t-lead" style={{ color: 'var(--txt-muted)', marginBottom: '1.5rem' }}>
            ...
            {/* Spec Card */}
            <div style={{ background: 'var(--clr-white)', padding: '2.5rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-light)' }}>
              <h2 className="h3" style={{ marginBottom: '1.5rem' }}>Charging Details</h2>
              
              <div className="info-strip">
                <div className="info-row">
                  <span className="info-label" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}><Clock size={16} /> Hours</span>
                  <span className="info-value">{outlet.specs.operatingHours}</span>
                </div>
                <div className="info-row">
                  <span className="info-label" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}><Zap size={16} /> Power</span>
                  <span className="info-value">{outlet.specs.powerPerCharger} DC</span>
                </div>
                <div className="info-row">
                  <span className="info-label" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}><Cable size={16} /> Connector</span>
                  <span className="info-value">{outlet.specs.connectorType}</span>
                </div>
                <div className="info-row" style={{ borderBottom: 'none' }}>
                  <span className="info-label" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}><Car size={16} /> Capacity</span>
                  <span className="info-value">{outlet.specs.maxSimultaneousEVs} EVs Simultaneously</span>
                </div>
              </div>
            </div>
            ...
            {/* Amenities Card */}
            <div style={{ background: 'var(--clr-white)', padding: '2.5rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-light)' }}>
              <h2 className="h3" style={{ marginBottom: '1.5rem' }}>Amenities</h2>
              <ul style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', rowGap: '1.75rem', columnGap: '1rem' }}>
                {outlet.amenities.map((item, idx) => (
                  <li key={item.id} className="amenity-item-hover" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--txt-medium)' }}>
                    <div className="amenity-icon-wrapper" style={{ color: 'var(--clr-ranger-green)', transition: 'color 0.2s ease' }}>{getAmenityIcon(item.id)}</div>
                    <span className="amenity-text" style={{ fontWeight: '500', transition: 'color 0.2s ease' }}>{item.name}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div style={{ position: 'sticky', top: '90px' }}>
            <div style={{ background: 'var(--clr-white)', padding: '1rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-light)' }}>
              <iframe
              ...
              <div style={{ marginTop: '1.25rem', padding: '0 1rem 0.5rem' }}>
                <h3 className="h4" style={{ marginBottom: '0.5rem' }}>Address</h3>
                <p style={{ color: 'var(--txt-muted)', fontSize: '0.9rem', lineHeight: '1.6' }}>
                  {outlet.fullAddress}
                </p>
      ...
      <div className="sticky-actions">
        <div className="sticky-actions__grid">
          ...
          <a href={`tel:${companyInfo.contact.primaryPhone}`} className="btn btn--outline" style={{ background: 'var(--bg-dark-surface)', borderColor: 'var(--border-on-dark)' }} aria-label="Call support">
            <Phone size={16} aria-hidden="true" />
            Call Support
          </a>
          <a href={`https://wa.me/${companyInfo.contact.whatsappNumber.replace('+', '')}`} target="_blank" rel="noopener noreferrer" className="btn btn--outline" style={{ background: 'var(--bg-dark-surface)', borderColor: 'var(--border-on-dark)', color: '#25D366' }} aria-label="WhatsApp chat (opens in new tab)">
